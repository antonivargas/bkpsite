jQuery(document).ready(function($) {
   ...
   'use strict';
    ...
    $('#countdown').countdown({until: new Date(2014, 11-1, 14)}); // enter event day
...
$(window).load(function(){
	...
	var newsletter_send = function(){
		...
		if ( email=="" ){ alert("Todos os campos devem ser preenchidos!"); $("#newsletter_email").focus(); }
		else {
			$.post("newsletter.send.php", { email:email }, function( result ){
				...
				if ( result=="SUCCESS" ){
					alert("Obrigado! Agora você faz parte de nossa base.");
					...
					alert("Erro desconhecido. Não foi possível adicionar seus dados.");
					$("#newsletter_email").focus();
				}
    ...
    var myOptions = {
        zoom: 14,
        center: new google.maps.LatLng(-26.8017, -48.617572,17), //change the coordinates
    ...
    marker = new google.maps.Marker({
        map: map,
        position: new google.maps.LatLng(-26.8017, -48.617572,17) //change the coordinates
    });
    infowindow = new google.maps.InfoWindow({
        content: "<b>Dream Valley Festival</b><br/>Santa Catarina - Brasil"  //add your address
    });
